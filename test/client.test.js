...
test('unified refresh keeps model and quota actions together', async () => {
  ...
  assert.match(host, /case 'refresh':\s*\{[\s\S]*const accounts = await refreshAccounts\(signal\)/)
  ...
  assert.match(source, /modelRefreshFailed/)
})
...
test('ordinary model selector filters image-only entries via the shared catalog predicate', async () => {
  ...
  assert.match(source, /function visibleModelsOf\(models: readonly ModelCatalogModel\[\], capabilities: readonly CpaModelCapability\[\]\): ModelCatalogModel\[\] \{\s*return models\.filter\(model => !isImageOnlyModel\(model\.id\) && !hasImageGenerationCapability\(model\.id, capabilities\)\)\s*\}/)
  assert.match(source, /if \(group\.id !== cpaProviderId\) \{\s*const models = visibleModelsOf\(group\.models, \[\]\)/)
  assert.match(source, /for \(const model of visibleModelsOf\(group\.models, capabilities\)\)/)
...
test('model settings hide image-only rows while preserving full draft data and extras on save', async () => {
  ...
  assert.match(source, /function visibleModelEntries\(models: readonly CpaModelDraft\[\]\): Array<\{ index: number; model: CpaModelDraft \}> \{\s*return models\.flatMap\(\(model, index\) => isImageOnlyDraft\(model\) \? \[\] : \[\{ index, model \}\]\)\s*\}/)
  assert.match(source, /function isImageOnlyDraft\(model: CpaModelDraft\): boolean \{\s*return isImageOnlyModel\(\{ \.\.\.model\.extraFields, id: model\.id \}\)\s*\}/)
