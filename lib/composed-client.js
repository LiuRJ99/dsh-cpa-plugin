window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const EMPTY_CPA_STATE = { accounts: [], status: 'idle', quotaFetchedAt: undefined, refreshIntervalMs: 300000 }
        const inject = ['connection', 'remote', 'remote.session', 'remote.settings', 'remote.credentials', 'remote.llm', 'slots', 'locale', 'configForms']
    ...
    async function installInitialProfile(remote, scope, baseURL, apiKey, messages) {
      const described = unwrapRpc(await remote.settings.describe())
      ...
      const credentialResult = unwrapRpc(await remote.credentials.describe([CREDENTIAL_REF]))
      const credential = credentialResult[CREDENTIAL_REF] || { configured: false }
      const discovered = unwrapRpc(await remote.llm.discoverModels(DISCOVERY_NS, {
        provider: PROVIDER,
        baseURL,
        api: 'openai-responses',
        ...(apiKey ? { apiKey } : {}),
      ...
      if (apiKey) unwrapRpc(await remote.credentials.set(CREDENTIAL_REF, apiKey))
      ...
      const existingBaseURL = typeof existingProfile?.baseURL === 'string'
        ...
        : undefined
      const updated = unwrapRpc(await remote.settings.mutate(
        PI_NS,
        [{
          op: 'set',
    ...
    async function credentialStatusOf(remote, ref = CREDENTIAL_REF) {
      try {
        const described = unwrapRpc(await remote.credentials.describe([ref]))
        return described[ref]?.configured === true
          ? 'configured'
          : 'missing'
      } catch {
        return 'unknown'
    ...
    function SettingsTab({ connection, remote, scope, t, cpa }) {
      ...
      useEffect(() => {
        ...
        const dispose = remote.$on('credentials/reference-updated', (ref) => {
          ...
          if (ref === MANAGEMENT_CREDENTIAL_REF) {
            void credentialStatusOf(remote, MANAGEMENT_CREDENTIAL_REF).then(status => {
       ...
       const submit = async (event) => {
        ...
        try {
          ...
          if (nextManagementKey) {
            unwrapRpc(await remote.credentials.set(MANAGEMENT_CREDENTIAL_REF, nextManagementKey))
          }
    ...
    function apply(ctx) {
      ...
      ctx.slots.inject(SETTINGS_SLOT, () => ctx.slots.register({
        ...
        inject: () => ({ connection: ctx.get('connection'), remote, scope, cpa }),
      }, SettingsTab))
    }
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CpaAutoRefresh = class {
			...
			armedInterval = 0;
			unsubscribe;
			constructor(cpa, schedule = defaultSchedule, cancel = defaultCancel) {
				this.cpa = cpa;
			...
			arm() {
				...
				if (this.timer !== void 0 && interval !== this.armedInterval) {
					this.cancel(this.timer);
					this.timer = void 0;
				}
		...
		*/
		function accountQuotaPercent(progress) {
		...
		*/
		...
		function accountCumulativeStats(account) {
		...
		*/
		function accountQuotaProgress(quota, t, model) {
			if (quota?.windows !== void 0 && quota.windows.length > 0) return windowProgress(deduplicateQuotaWindows(quota.windows, model), t);
		...
		function windowProgress(windows, t) {
		...
		function deduplicateQuotaWindows(windows, model) {
			const scoped = model === void 0 ? windows : windows.filter((window) => windowMatchesModelPool(window, model));
		...
		*/
		function windowMatchesModelPool(window, model) {
			...
			return hints === void 0 || hints.some((hint) => group.includes(hint));
		}
		...
		function accountAvailabilityLabel(account, t, model) {
		...
		*/
		function accountAvailability(account, model) {
		...
		function quotaLooksInsufficient(account, model) {
			const allWindows = account.quota?.windows ?? [];
			const poolWindows = allWindows.filter((window) => windowMatchesModelPool(window, model));
			if ((model !== void 0 && poolWindows.length === 0 ? allWindows : poolWindows).some((window) => window.exceeded === true || window.remaining <= 0 || window.total !== void 0 && window.total > 0 && window.remaining / window.total <= .2)) return true;
			...
			return /(quota|limit|exhaust|insufficient|balance|credit|rate.?limit|too many|429)/i.test(`${account.status}`);
		}
		...
		*/
		...
		function CpaAccountIndicator({ cpa, directory, sessionId, t }) {
			...
			const quotaLabel = percent !== void 0 ? `${Math.round(percent)}%` : progress.length === 0 ? t("account.quotaUnknown") : progress.map(formatProgress).join(" · ");
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: "dsh-cpa-account-indicator-label",
							children: accountLabel(account)
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
							className: "dsh-cpa-account-indicator-quota",
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "dsh-cpa-quota-full",
								children: quotaLabel
							}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "dsh-cpa-quota-short",
								children: shortQuotaLabel
							})]
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					...
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: "dsh-cpa-account-menu-note",
							role: "note",
							children: t("account.selectionInfo")
						}),
						liveSupported.map((option) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(AccountOption, {
							account: option,
							model,
		...
		function AccountOption({ account, model, selected, onChoose, t }) {
			const availability = accountAvailability(account, model);
			const quotaEntry = accountQuotaPercentEntry(accountQuotaProgress(account.quota, t, model));
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						...
						style: quotaEntry?.percent === void 0 ? void 0 : { width: `${quotaEntry.percent}%` }
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
						className: "dsh-cpa-account-option-copy",
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("strong", {
							className: "dsh-cpa-account-option-title",
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: accountLabel(account) }),
								stats.success > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
									className: "dsh-cpa-count is-success",
									children: ["· ", stats.success]
								}) : null,
								stats.failed > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
									className: "dsh-cpa-count is-failed",
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						...
						children: quotaEntry?.percent === void 0 ? t("account.quotaUnknown") : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: "dsh-cpa-account-option-quota-label",
							children: quotaEntry.label
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
							className: "dsh-cpa-account-option-quota-value",
							children: [Math.round(quotaEntry.percent), "%"]
						})] })
					}),
		...
		function currentAccount(accounts, model, selected) {
			if (selected !== void 0) {
		...
		*/
		...
		var CpaClient = class {
			...
			*/
			...
			async listModelCapabilities() {
				...
				return (await this.loadModelCapabilities()).map((model) => ({
					provider,
					model: model.id,
			...
			hasSpeedPreference(sessionId, model) {
			...
			async syncSpeed(sessionId, model) {
				const value = await this.call("session-speed", {
					sessionId,
					model
				});
				if (value.speed === void 0) return void 0;
				const preferences = {
		...
		function hasFastSpeedCapability(model, capabilities) {
			...
			return capabilities.some((entry) => {
				return [entry.id, ...entry.aliases ?? []].some((id) => isCodexResponsesModel(id) && id.trim().toLowerCase() === wanted) && entry.serviceTiers.some((tier) => tier.id === "priority");
			});
		}
		function hasImageGenerationCapability(model, capabilities) {
			const wanted = model.trim().toLowerCase();
			if (wanted === "") return false;
			return capabilities.some((entry) => entry.imageGeneration === true && [entry.id, ...entry.aliases ?? []].some((id) => id.trim().toLowerCase() === wanted));
		}
		...
		function mergeAccountSnapshots(previous, next) {
			...
			return next.map((account) => {
				...
				return {
					...account,
					...quotaStale ? { quotaStale: true } : {},
					...account.plan === void 0 && old.plan !== void 0 ? { plan: old.plan } : {},
					...account.quota === void 0 && old.quota !== void 0 && quotaStale ? { quota: old.quota } : {}
				};
			});
		}
		...
		*/
		function imageModelInfoOf(value) {
			...
			const edit = booleanValue(entry?.image_edit, entry?.imageEdit, entry?.supports_image_edit, entry?.supportsImageEdit, imageCapabilities?.image_edit, imageCapabilities?.imageEdit, imageCapabilities?.edit) ?? fallback?.edit;
			return {
		...
		function CpaModelSelect({ locked, available, directory, load, select, cpa, sessionId, session, useChat, t }) {
			...
			const hasImages = (0, react.useSyncExternalStore)((listener) => session?.subscribe(listener) ?? (() => {}), () => session !== void 0 && snapshotHasImages(session.getSnapshot()), () => false) || chatNodes.some((node) => hasImageContent(node.content));
			...
			const displayGroups = (0, react.useMemo)(() => displayModelGroups(state.groups, cpaState.providerId, cpaState.modelCapabilities, t), [
				state.groups,
				cpaState.providerId,
				cpaState.modelCapabilities,
				t
			]);
			...
			(0, react.useEffect)(() => {
				...
				cpa.syncSpeed(sessionId, current.model).catch(() => {});
			}, [
				available,
				cpa,
				isCpa,
			...
			const choose = (selection) => {
				(async () => {
					...
					if ((await select(nextSelection))?.ok !== true) return;
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconChevronDownOutlineRegular, { className: `dsh-cpa-chevron${open ? " is-open" : ""}` })
					]
				}), open ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					id: `${id}-menu`,
					...
					children: [
						...
						pane === "model" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
								...
								state.status === "ready" && displayGroups.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									className: "dsh-cpa-status",
		...
		function MenuRow(props) {
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconChevronRightOutlineRegular, { className: "dsh-cpa-menu-chevron" })
				]
			});
		}
		...
		function displayModelGroups(groups, cpaProviderId, capabilities, t) {
		...
		function visibleModelsOf(models, capabilities) {
		...
		async function ensureDefaultAccountForModel(cpa, sessionId, modelId) {
			...
			const ordered = current !== void 0 && accountAvailability(current, modelId) === "available" ? [current, ...available.filter((account) => account.authIndex !== current.authIndex)] : available;
			...
			const currentAvailable = current !== void 0 && accountAvailability(current, modelId) === "available" ? current : void 0;
		...
		var CpaModelSettingsController = class {
			...
			constructor(ctx, settings, cpa) {
				...
				settings.subscribe(() => {
					...
					else if (!this.isDirty()) this.load();
					else this.publish();
				});
				cpa.store.subscribe(() => {
			...
			inject() {
				return {
					...
					editModel: (index, field, value) => {
						...
						if (actualIndex === void 0) return;
						const model = this.draft.models[actualIndex];
						if (model === void 0) return;
						this.draft.models[actualIndex] = {
							...model,
							[field]: value
						};
						this.error = null;
			...
			async load() {
				...
				try {
					...
					this.namespace = snapshot.value === void 0 ? void 0 : {
			...
			*/
			async ensureReasoningConfiguration(profile) {
				...
				this.migrating = true;
				try {
					await this.settings.mutate(ops, this.revision);
					const snapshot = this.settings.getSnapshot();
					this.namespace = snapshot.value === void 0 ? void 0 : {
						value: snapshot.value,
						revision: snapshot.revision ?? this.revision
					};
		...
		function visibleModelEntries(models) {
			return models.flatMap((model, index) => isImageOnlyDraft(model) ? [] : [{
				index,
				model
			}]);
		}
		function isImageOnlyDraft(model) {
			return isImageOnlyModel({
				...model.extraFields,
				id: model.id
			});
		}
		function extraModelFields(value) {
			const raw = valueObject(value);
			if (raw === void 0) return {};
			return Object.fromEntries(Object.entries(raw).filter(([key]) => key !== "id" && key !== "name" && key !== "contextWindow" && key !== "maxTokens" && key !== "reasoningEfforts"));
		}
		...
		var CpaSettingsCardController = class {
			...
			makeStore() {
				return (0, _deepseek_ai_dsh_client_store.createSnapshotStore)(this.projection());
			}
			projection() {
		...
		function AccountRow({ account, t }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				}), stats.hasRecords ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
					className: "dsh-cpa-account-stats-badges",
					title: `${status} · ${stats.success} / ${stats.failed}`,
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
						className: "dsh-cpa-stat-badge is-success",
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: "dsh-cpa-badge-dot is-success",
							"aria-hidden": "true"
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: stats.success })]
					}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
						className: "dsh-cpa-stat-badge is-failed",
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: "dsh-cpa-badge-dot is-failed",
							"aria-hidden": "true"
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: stats.failed })]
					})]
				}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
...
`;
		...
		*/
		const SETTINGS_NAV_MARKER = "data-dsh-cpa-settings-nav";
		/**
		...
		*/
		function applyAdditive(ctx) {
			...
			ctx.provide(MODEL_CAPABILITY_SERVICE, { listModelCapabilities: () => cpa.listModelCapabilities() });
			cpa.loadConfig().then(() => {
				cpa.refresh().catch(() => {});
				cpa.loadModelCapabilities().catch(() => {});
				cpa.loadInputCapabilities().catch(() => {});
		...
		function apply(ctx) {
			...
			ctx.inject(["slots", "configForms"], (scope) => {
				const model = new CpaModelSettingsController(ctx, scope.configForms.get("llm-pi-ai"), cpa);
