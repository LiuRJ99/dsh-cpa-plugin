...
import { CONTEXT_WINDOW_EXCEEDED_CODE, CallId, EMPTY_RESPONSE_CODE, LlmError, QUOTA_EXCEEDED_CODE, attributionHeaders, contentHasImage, isContextWindowExceededError, isQuotaExceededError } from "@deepseek-ai/dsh-llm";
...
var AbstractChatCompletionRunner = class extends EventStream$1 {
	constructor() {
		super(...arguments);
	...
	[(_ChatCompletionStream_params = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_choiceEventStates = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_currentChatCompletionSnapshot = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_instances = /* @__PURE__ */ new WeakSet(), _ChatCompletionStream_beginRequest = function _ChatCompletionStream_beginRequest() {
		...
		for (const { delta, finish_reason, index, logprobs = null, ...other } of chunk.choices) {
			...
			if (content) {
				...
				if (!choice.message.refusal && __classPrivateFieldGet(this, _ChatCompletionStream_instances, "m", _ChatCompletionStream_getAutoParseableResponseFormat).call(this)) choice.message.parsed = partialParse$1(choice.message.content);
			}
			if (tool_calls) {
				...
				for (const { index, id, type, function: fn, ...rest } of tool_calls) {
					...
					if (fn?.arguments) {
						...
						if (shouldParseToolCall(__classPrivateFieldGet(this, _ChatCompletionStream_params, "f"), tool_call)) tool_call.function.parsed_arguments = partialParse$1(tool_call.function.arguments);
					}
				}
			}
		}
...
var AssistantStream = class extends EventStream$1 {
	constructor() {
		super(...arguments);
...
var ResponseStream = class ResponseStream extends EventStream$1 {
	constructor(params) {
		super();
...
async function processResponsesStream(openaiStream, output, stream, model, options) {
	...
	const finalizeResponse = (response) => {
		...
		output.stopReason = mapStopReason$1(response?.status);
...
function mapStopReason$1(status) {
...
//#endregion
//#region ../../codex-work/deepseek-harness/packages/llm/llm-pi-ai/src/replay.ts
/**
...
//#endregion
//#region ../../codex-work/deepseek-harness/packages/llm/llm-pi-ai/src/context.ts
/**
...
//#endregion
//#region ../../codex-work/deepseek-harness/node_modules/.pnpm/@earendil-works+pi-ai@0.82.1_@modelcontextprotocol+sdk@1.29.0_zod@4.4.3__ws@8.21.0_zod@4.4.3/node_modules/@earendil-works/pi-ai/dist/utils/overflow.js
/**
...
//#endregion
//#region ../../codex-work/deepseek-harness/packages/llm/llm-pi-ai/src/stream.ts
/**
...
function reasoningEfforts(configured, model) {
	if (configured?.reasoningEfforts !== void 0) return [...configured.reasoningEfforts];
	if (/^gpt-5\.6(?:-|$)/i.test(model)) return [
		"low",
		"medium",
		"high",
		"max"
...
*/
async function discoverCpaModels(request, resolveStoredApiKey) {
...
function apply(ctx, config) {
	...
	if (config.registerDiscovery !== false) ctx.inject(["llm"], (scope) => {
		scope.llm.registerModelDiscovery(MODEL_DISCOVERY_NS, (request) => discoverCpaModels(request, () => readCredential(MODEL_KEY_REF)));
	});
		...
		scope.get("connection").rpc.handle("/cpa", async (endpoint, payload, signal) => {
			switch (endpoint) {
				...
				case "refresh": {
					await refreshModelCatalog(ctx, signal);
					const current = await readAccounts(signal);
					return ok(current.quotaFetchedAt === void 0 ? await refreshAccounts(signal) : current);
				}
				case "account-models": {
				...
				case "model-capabilities": {
					fastModelIds.clear();
					const value = await fetchModelCapabilities(ctx, effectiveConfig(ctx, config), readCredential, signal);
					for (const model of value.models) if (model.serviceTiers.some((tier) => tier.id === "priority")) fastModelIds.add(model.id);
					return ok(value);
				}
...
async function fetchModelCapabilities(ctx, config, readCredential, signal) {
	...
	if (key === void 0 || key.trim() === "") return configuredModelCapabilities(ctx, config);
	...
	for (const model of configuredModelCapabilities(ctx, config).models) {
		...
		if (current === void 0) byId.set(model.id, model);
...
function configuredModelCapabilities(ctx, config) {
	return {
		models: cpaFastRoute(ctx, config)?.models.flatMap((model) => isGpt56Model(model.id) ? [{
			id: model.id,
			serviceTiers: [{
				id: "priority",
				name: "Fast"
...
function parseModelCapabilities(value) {
	...
	return (Array.isArray(root?.models) ? root.models : Array.isArray(root?.data) ? root.data : []).flatMap((entryValue) => {
		...
		return [{
			id,
			serviceTiers: parsedTiers.length > 0 ? parsedTiers : isGpt56Model(id) ? [{
				id: "priority",
				name: "Fast"
			}] : []
		}];
...
function parseCodexQuota(body) {
	...
	const remaining = used === void 0 ? void 0 : Math.max(0, 100 - used);
	const limitReached = booleanValue(rateLimit?.limit_reached) === true || booleanValue(rateLimit?.limitReached) === true;
	const balance = firstNumber(credits?.balance);
	const exceeded = limitReached || booleanValue(credits?.overage_limit_reached) === true || booleanValue(spendControl?.reached) === true || remaining !== void 0 && remaining <= 0 || remaining === void 0 && balance !== void 0 && balance <= 0;
	...
	const quota = used !== void 0 && remaining !== void 0 ? {
		remaining,
		total: 100,
		used,
		unit: "%",
...
function codexWindowKey(seconds) {
	if (seconds === void 0) return "quota";
