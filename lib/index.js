...
import { CONTEXT_WINDOW_EXCEEDED_CODE, EMPTY_RESPONSE_CODE, IMAGE_OFFLOAD_REQUIRED_CODE, LlmError, QUOTA_EXCEEDED_CODE, ToolCallId, attributionHeaders, contentHasImage, isContextWindowExceededError, isQuotaExceededError, offloadedImageText, projectOffloadedImages, requiredImageOffload } from "@deepseek-ai/dsh-llm";
import { isContextOverflow } from "@earendil-works/pi-ai";
//#region src/catalog.js
const IMAGE_MODEL_FALLBACKS = /* @__PURE__ */ new Map([
	["gpt-image-1.5", {
		engine: "gpt",
		edit: true
	}],
...
const IMAGE_ENGINE_VALUES = /* @__PURE__ */ new Set(["gpt", "gemini"]);
...
*/
function imageModelInfoOf(value) {
	const entry = record$1(value);
	const id = typeof value === "string" ? modelIdOf({ id: value }) : modelIdOf(value);
	const capabilities = record$1(entry?.capabilities);
	const imageCapabilities = record$1(entry?.image_capabilities) ?? record$1(entry?.imageCapabilities) ?? capabilities;
	const explicitGeneration = booleanValue$1(entry?.image_generation, entry?.imageGeneration, entry?.supports_image_generation, entry?.supportsImageGeneration, imageCapabilities?.image_generation, imageCapabilities?.imageGeneration, imageCapabilities?.generate, imageCapabilities?.generation) ?? (hasImageOutput(entry?.output_modalities) || hasImageOutput(imageCapabilities?.output_modalities) ? true : void 0);
	const fallback = id === void 0 ? void 0 : IMAGE_MODEL_FALLBACKS.get(id);
	const namespace = imageNamespaceInfo(id);
	const resolvedGeneration = explicitGeneration ?? (namespace === void 0 ? void 0 : true);
	if (resolvedGeneration === false) return void 0;
	if (resolvedGeneration !== true && fallback === void 0) return void 0;
	const engine = imageEngineValue(entry?.image_engine, entry?.imageEngine, imageCapabilities?.image_engine, imageCapabilities?.imageEngine, imageCapabilities?.engine) ?? fallback?.engine ?? namespace?.engine;
	const edit = booleanValue$1(entry?.image_edit, entry?.imageEdit, entry?.supports_image_edit, entry?.supportsImageEdit, imageCapabilities?.image_edit, imageCapabilities?.imageEdit, imageCapabilities?.edit) ?? fallback?.edit;
	return {
		imageGeneration: true,
		...engine === void 0 ? {} : { imageEngine: engine },
		...edit === void 0 ? {} : { imageEdit: edit }
	};
}
...
function booleanValue$1(...values) {
...
function assertSupportedHistory(messages) {
	for (const message of messages) {
		...
		if (message.content.some((block) => block.type === "tool-addition" || block.type === "tool-removal")) throw new LlmError("Tool-change blocks are not supported yet", "UNSUPPORTED_CONTENT");
...
function piContext(options, messages, systemPrompt) {
	...
	for (const message of history) {
		...
		if (message.role === "tool") {
			messages.push({
				...
				content: [{
					type: "text",
					text: flattenText(message) || "(no output)"
				}],
...
function splitSystemPrompt(options) {
	...
	return {
		messages: rest,
		...prompt.length > 0 ? { systemPrompt: prompt } : {}
	};
}
...
async function toPiContextWithImages(options, attachments, onReplayDegrade, maxRequestImageBytes) {
	...
	if (maxRequestImageBytes !== void 0) {
		...
		if (offloadImages > 0) throw new LlmError(`pi-ai request images exceed the ${maxRequestImageBytes}-byte base64 bound`, IMAGE_OFFLOAD_REQUIRED_CODE, { offloadImages });
	}
...
function isCodexResponsesModel(id) {
...
async function* streamCpaFast(options, route, resolveApiKey, resolveAttachments, serviceTier) {
	...
	const requestedEffort = options.reasoningEffort === void 0 ? route.reasoning === void 0 ? void 0 : String(route.reasoning) : String(options.reasoningEffort);
	if (requestedEffort !== void 0 && requestedEffort !== "off" && (!Object.prototype.hasOwnProperty.call(effortMap, requestedEffort) || effortMap[requestedEffort] === null)) throw new LlmError(`CLIProXyAPI model "${options.model}" does not support reasoning effort "${requestedEffort}"`, "UNSUPPORTED_REASONING_EFFORT");
	...
	const apiKey = (route.apiKeyEnv === void 0 ? void 0 : await resolveApiKey(route.apiKeyEnv))?.trim() || PLACEHOLDER_API_KEY;
	yield* toStreamChunks((await loadCodexStream(route.provider))(model, context, {
		...apiKey === void 0 ? {} : { apiKey },
		...reasoningEffort === void 0 ? {} : { reasoningEffort },
		...options.temperature === void 0 ? {} : { temperature: options.temperature },
		...options.maxTokens === void 0 ? {} : { maxTokens: options.maxTokens },
		...maxOutputTokens === void 0 ? {} : { onPayload: (payload) => addMaxOutputTokens(payload, maxOutputTokens) },
...
function cpaModel(route, configured, id, effortMap) {
	const efforts = supportedReasoningEfforts(effortMap);
	const thinkingLevelMap = Object.keys(effortMap).length === 0 ? void 0 : effortMap;
	return {
		id,
...
function reasoningEfforts(configured) {
	...
	if (configuredEfforts === void 0 || configuredEfforts === false) return {};
	if (Array.isArray(configuredEfforts)) return Object.fromEntries(configuredEfforts.filter((effort) => THINKING_LEVELS.has(effort)).map((effort) => [effort, effort]));
...
function supportedReasoningEfforts(effortMap) {
...
*/
function codexReasoningEffortOf(effortMap, requestedEffort) {
	const effective = requestedEffort ?? (typeof effortMap.off === "string" ? "off" : void 0);
	if (effective === void 0) return void 0;
	if (effective === "off") return typeof effortMap.off === "string" ? "none" : void 0;
	return effective;
}
...
function addMaxOutputTokens(payload, maxOutputTokens) {
...
function codexBaseURL(baseURL) {
	const normalized = baseURL.trim().replace(/\/+$/, "");
	if (normalized.endsWith("/codex/responses") || normalized.endsWith("/codex") || normalized.endsWith("/backend-api")) return normalized;
	return `${normalized.endsWith("/v1") ? normalized.slice(0, -3) : normalized}/backend-api`;
}
...
function patchWebSocketOnlyTransport(source) {
	const sessionIdExpression = String.raw`(?:options\?\.sessionId|cacheSessionId)`;
	const disabledForSession = new RegExp(String.raw`const websocketDisabledForSession\s*=\s*transport !== "sse" && isWebSocketSseFallbackActive\(${sessionIdExpression}\);`);
	const retryVariables = /let retriedWebSocketConnectionLimit\s*=\s*false;/;
	const connectionLimitRetry = /if \(!aborted && connectionLimitBeforeStart && !retriedWebSocketConnectionLimit\) \{\s*retriedWebSocketConnectionLimit = true;\s*continue;\s*\}/;
	const websocketFailureHandling = new RegExp(String.raw`if \(aborted \|\| \(isCodexNonTransportError\(error\) && !connectionLimitBeforeStart\)\) \{[\s\S]*?recordWebSocketFailure\((${sessionIdExpression}), error\);[\s\S]*?recordWebSocketSseFallback\(\1\);\s*break;`);
	...
	for (const fragment of [fallbackSessionRecord, fallbackActiveRecord]) if (!source.includes(fragment)) throw new Error("openai-codex-responses source no longer supports the WebSocket-only transport patch");
	for (const pattern of [
		disabledForSession,
		retryVariables,
		connectionLimitRetry,
		websocketFailureHandling
	]) if (!pattern.test(source)) throw new Error("openai-codex-responses source no longer supports the WebSocket-only transport patch");
	return source.replace(disabledForSession, "const websocketDisabledForSession = false;").replace(retryVariables, `let websocketRetries = 0;
                const maxWebSocketRetries = Number.isFinite(options?.maxRetries)
                    ? Math.min(Math.max(0, Math.floor(options.maxRetries)), 5)
                    : 3;`).replace(connectionLimitRetry, "").replace(websocketFailureHandling, (_match, activeSessionId) => `if (aborted || (isCodexNonTransportError(error) && !connectionLimitBeforeStart)) {
                            throw error;
                        }
                        ...
                        throw error;`).replace(fallbackSessionRecord, "").replace(fallbackActiveRecord, "stats.websocketFallbackActive = false;");
}
function patchCodexSource(source, providerIds) {
	if (!/function extractAccountId\(token\) \{/.test(source)) throw new Error("openai-codex-responses source no longer contains extractAccountId(token)");
	let patched = source.replace(/function extractAccountId\(token\) \{[\s\S]*?\n\}/, EXTRACT_ACCOUNT_ID_PATCH);
	if (!patched.includes("headers.set(\"chatgpt-account-id\", accountId);")) throw new Error("openai-codex-responses source no longer sets chatgpt-account-id");
	patched = patched.replace("headers.set(\"chatgpt-account-id\", accountId);", "if (accountId) {\n        headers.set(\"chatgpt-account-id\", accountId);\n    }");
	const providersPattern = /const CODEX_TOOL_CALL_PROVIDERS = new Set\(\[([^\]]*)\]\);/;
	const providersMatch = patched.match(providersPattern);
	if (!providersMatch) throw new Error("openai-codex-responses source no longer defines CODEX_TOOL_CALL_PROVIDERS");
	const existing = providersMatch[1];
	const extras = providerIds.filter((provider) => provider.trim() !== "").map((provider) => JSON.stringify(provider.trim())).join(", ");
	...
	if (!customApi.includes(`api: ${JSON.stringify("cliproxyapi-codex-responses")}`)) throw new Error("openai-codex-responses source no longer exposes its assistant api metadata");
	return patchWebSocketOnlyTransport(customApi).replace(/^\/\/# sourceMappingURL=.*$/gm, "");
}
function rewriteRelativeImports(source, originalDir) {
	return source.replace(/from\s+"((?:\.\.?\/)[^"]+)"/g, (_full, relativePath) => {
		return `from ${JSON.stringify(pathToFileURL(join(originalDir, relativePath)).href)}`;
	});
}
function resolveCodexModuleFromNodeEntry(entryPath) {
	try {
...
function resolveCodexModulePath() {
	const candidates = [];
	try {
	...
	for (const path of candidates) if (existsSync(path)) return {
		path,
		dir: dirname(path)
	};
	throw new Error(`Cannot resolve openai-codex-responses.js (tried: ${candidates.join(", ") || "none"})`);
}
const codexStreams = /* @__PURE__ */ new Map();
async function loadCodexStream(provider) {
...
async function importCodexStream(provider) {
	const { path, dir } = resolveCodexModulePath();
	const patched = rewriteRelativeImports(patchCodexSource(readFileSync(path, "utf8"), [provider]), dir);
	...
	if (typeof module.stream !== "function") throw new Error("patched openai-codex-responses module has no stream export");
	...
	for (const entryValue of entries) {
		...
		models.push({
			...
			...imageInfo?.imageGeneration !== true ? {} : {
				imageGeneration: true,
				...imageInfo.imageEngine === void 0 ? {} : { imageEngine: imageInfo.imageEngine },
				...imageInfo.imageEdit === void 0 ? {} : { imageEdit: imageInfo.imageEdit }
			}
		});
	}
	return models;
...
const Config = z.object({
	...
	refreshIntervalMs: z.natural().min(0).default(3e5).volatile(),
	...
	*/
	...
	const applyCapabilities = (value, key) => {
		...
		for (const model of value.models) {
			...
			for (const alias of model.aliases ?? []) if (isCodexResponsesModel(alias) && !isImageOnlyModel(alias)) fastModelIds.add(alias);
		}
		return value;
	};
	...
	ctx.on(MODEL_REFRESH_EVENT, () => invalidateModelCapabilities());
	...
	const listImageModels = async (signal) => {
		return mergeImageModels(imageModelsOf((await loadModelCapabilities(signal ?? new AbortController().signal)).models), imageModelsFromProfile(ctx, effectiveConfig(ctx, config)));
	};
	...
	}, readImageCredential, { listModels: (signal) => listImageModels(signal) });
	...
	ctx.provide(MODEL_CAPABILITY_SERVICE, { listModelCapabilities: async (signal) => {
		...
		return value.models.flatMap((model) => [model.id, ...model.aliases ?? []].map((modelId) => ({
			...
			...model.imageEngine === void 0 ? {} : { imageEngine: model.imageEngine },
			...model.imageEdit === void 0 ? {} : { imageEdit: model.imageEdit }
		})));
	} });
	...
	const handleCpaStream = (options, next) => {
		...
		if (!(fastModelIds.has(options.model) || isCodexResponsesModel(options.model))) return next();
		const extension = options;
		const key = options.sessionId === void 0 ? void 0 : speedKey(String(options.sessionId), options.model);
		const requestedTier = extension.serviceTier === PRIORITY_SERVICE_TIER;
		const fast = requestedTier || key !== void 0 && speedBySessionModel.get(key) === "fast";
		if (requestedTier && key !== void 0) speedBySessionModel.set(key, "fast");
		const route = cpaFastRoute(ctx, currentConfig);
		if (route === void 0) return next();
		return streamCpaFast(options, route, readCredential, () => ctx.get("attachments"), fast ? PRIORITY_SERVICE_TIER : void 0);
	};
			...
			switch (endpoint) {
				...
				case "set-refresh-interval": {
					...
					return ok({
						...
						...modelRefreshError === void 0 ? {} : { modelRefreshError }
					});
				}
...
function effectiveConfig(ctx, config) {
	const providers = valueObject(valueObject(settingsEntryValue(ctx, MODEL_SETTINGS_NS))?.providers);
...
async function refreshModelCatalogWarning(ctx, signal) {
	try {
		await refreshModelCatalog(ctx, signal);
		return;
	} catch (error) {
		if (signal.aborted) throw signal.reason ?? error;
		const message = error instanceof AggregateError && error.errors.length > 0 ? error.errors.map((item) => item instanceof Error ? item.message : String(item)).join("; ") : error instanceof Error ? error.message : String(error);
		ctx.logger.warn("CLIProxyAPI model catalog refresh failed: " + message);
		return message;
	}
}
function effectiveRefreshInterval(ctx, config) {
	return normalizeRefreshInterval(valueObject(settingsEntryValue(ctx, REFRESH_SETTINGS_NS))?.refreshIntervalMs ?? config.refreshIntervalMs);
}
function settingsEntryValue(ctx, entryId) {
	return ctx.get("settings")?.describe().find((entry) => entry.ns === entryId)?.value;
}
function normalizeRefreshInterval(value) {
	if (value !== null && typeof value === "object" && "get" in value && typeof value.get === "function") value = value.get();
...
function cpaFastRoute(ctx, config) {
	const profile = valueObject(valueObject(valueObject(valueObject(settingsEntryValue(ctx, MODEL_SETTINGS_NS))?.providers))?.[config.providerId]);
	...
	const models = (Array.isArray(profile?.models) ? profile.models : []).flatMap((modelValue) => {
		...
		if (id === void 0) return [];
		const modelReasoning = model?.reasoningEfforts === false ? void 0 : reasoningMap(model?.reasoningEfforts);
		return [{
			id,
			...
			...modelReasoning === void 0 ? {} : { reasoningEfforts: modelReasoning },
...
function cpaImageRoute(ctx, config) {
	const providers = valueObject(valueObject(settingsEntryValue(ctx, MODEL_SETTINGS_NS))?.providers);
	const providerId = providerCandidates(config).find((candidate) => valueObject(providers?.[candidate]) !== void 0) ?? config.providerId;
...
function parseModelCapabilities(value) {
	...
	return (Array.isArray(root?.models) ? root.models : Array.isArray(root?.data) ? root.data : []).flatMap((entryValue) => {
		...
		return [{
			id,
			...firstStringValue(entry?.display_name, entry?.name, entry?.description) === void 0 ? {} : { name: firstStringValue(entry?.display_name, entry?.name, entry?.description) },
...
function imageModelsOf(models) {
	...
	return models.flatMap((model) => {
		...
		if (id === "" || id.length > 256 || name.length > 256 || engine === void 0 || seen.size >= 256 || seen.has(id.toLowerCase())) return [];
		...
		return [{
			...
			...model.imageEdit === void 0 ? {} : { supportsEdit: model.imageEdit }
		}];
	});
}
function imageModelsFromProfile(ctx, config) {
	const providers = valueObject(valueObject(settingsEntryValue(ctx, MODEL_SETTINGS_NS))?.providers);
	const providerId = providerCandidates(config).find((candidate) => valueObject(providers?.[candidate]) !== void 0) ?? config.providerId;
	const profile = valueObject(providers?.[providerId]);
	...
	return imageModelsOf(profile.models.flatMap((modelValue) => {
		...
		const id = stringValue(model.id);
		const imageInfo = imageModelInfoOf(model);
		if (id === void 0 || imageInfo?.imageGeneration !== true) return [];
		return [{
			id,
			...
			...imageInfo.imageEdit === void 0 ? {} : { imageEdit: imageInfo.imageEdit },
			imageGeneration: true,
			aliases: Array.isArray(model.aliases) ? model.aliases : void 0,
			serviceTiers: []
		}];
	}));
}
function mergeImageModels(primary, secondary) {
	const seen = /* @__PURE__ */ new Set();
...
function inferredImageEngine(id) {
...
async function fetchAntigravityQuotaSummary(urls, config, readCredential, account, headers, signal) {
	let lastError;
	for (const url of urls) try {
		return await callUpstream(config, readCredential, account.authIndex, "POST", url, headers, JSON.stringify({ project: account.projectId }), signal);
	} catch (error) {
		lastError = error;
	}
	throw lastError;
...
async function fetchAccountQuota(config, readCredential, account, signal) {
	...
	if (provider.includes("antigravity")) {
		...
		const quotaRequest = account.projectId === void 0 ? Promise.resolve({}) : fetchAntigravityQuotaSummary([
			"https://daily-cloudcode-pa.googleapis.com/v1internal:retrieveUserQuotaSummary",
			"https://daily-cloudcode-pa.sandbox.googleapis.com/v1internal:retrieveUserQuotaSummary",
			"https://cloudcode-pa.googleapis.com/v1internal:retrieveUserQuotaSummary"
		], config, readCredential, account, headers, signal);
	...
	if (provider === "kimi" || provider === "kimi-coding" || provider === "kimi.com" || provider === "kimi-ai" || provider === "kimi.ai") {
		const kimiHost = provider === "kimi-ai" || provider === "kimi.ai" ? "api.kimi.ai" : "api.kimi.com";
		return parseKimiQuota(await callUpstream(config, readCredential, account.authIndex, "GET", `https://${kimiHost}/coding/v1/usages`, {
			Authorization: "Bearer $TOKEN$",
			Accept: "application/json"
		}, void 0, signal));
	}
...
function parseKimiQuota(body) {
	...
	for (const [key, window] of [
		...
		if (ratio === void 0 || ratio < 0 || ratio > 1) continue;
	...
	const weekly = kimiCountWindow(valueObject(root.usage), "weekly");
	if (weekly !== void 0 && !windows.has("weekly")) windows.set("weekly", weekly);
	if (!windows.has("five_hour") && Array.isArray(root.limits)) for (const item of root.limits) {
		...
		if (duration === void 0 || !/^TIME_UNIT_MINUTE$/i.test(unit ?? "") || Math.abs(duration - 300) > 1) continue;
		const fiveHour = kimiCountWindow(valueObject(limit?.detail) ?? limit, "five_hour");
		if (fiveHour !== void 0) windows.set("five_hour", fiveHour);
		break;
	}
	const ordered = [
		"five_hour",
	...
	return { quota: {
		...
		windows: ordered,
		...primary.resetAt === void 0 ? {} : { resetAt: primary.resetAt },
		...primary.window !== "five_hour" ? {} : { windowSeconds: 18e3 }
	} };
}
function kimiCountWindow(value, window) {
	...
	if (total === void 0 || total <= 0 || reportedRemaining === void 0 && used === void 0) return void 0;
...
function reasoningMap(value) {
	...
	if (record === void 0) return void 0;
	const result = {};
	for (const [key, wire] of Object.entries(record)) if (wire === null || typeof wire === "string") result[key] = wire;
	return Object.keys(result).length === 0 ? void 0 : result;
}
...
//#endregion
export { Config, apply, inject, name, parseCodexQuota, parseKimiQuota, parseModelCapabilities };
