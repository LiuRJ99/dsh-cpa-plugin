...
function apply(ctx, config) {
	...
	const handleCpaStream = (options, next) => {
		...
		const requestedSpeed = extension.speed === "fast";
		if (requestedTier || requestedSpeed) speedBySessionModel.set(key, "fast");
		if (!requestedTier && !requestedSpeed && speedBySessionModel.get(key) !== "fast") return next();
...
function parseModelCapabilities(value) {
	...
	return (Array.isArray(root?.models) ? root.models : Array.isArray(root?.data) ? root.data : []).flatMap((entryValue) => {
		...
		return [{
			id,
			serviceTiers: (Array.isArray(entry?.service_tiers) ? entry.service_tiers : Array.isArray(entry?.serviceTiers) ? entry.serviceTiers : []).flatMap((tierValue) => {
...
//#endregion
export { Config, apply, inject, name, parseModelCapabilities };
